...
def relevant_user_tweets(relevant_tweets: list[str], irrelevant_tweets: list[str], original_query: list[float], original_query_str: list[str], df, index, vs, alpha, beta, gamma):
    """Returns new tweets based on relevant tweets provided by the user"""
    # df, index, vs = load_data()
    updated_query_v = rocchio(original_query, relevant_tweets, irrelevant_tweets, df, vs, alpha, beta, gamma)
